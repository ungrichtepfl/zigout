...
pub fn vecMult(vec: *const Vector2D, scalar: f32) Vector2D {
    return .{
        .x = @as(i32, @intFromFloat(@as(f32, @floatFromInt(vec.x)) * scalar)),
        .y = @as(i32, @intFromFloat(@as(f32, @floatFromInt(vec.y)) * scalar)),
    };
}

pub fn multToVec(vec: *Vector2D, scalar: f32) void {
    vec.x = @as(i32, @intFromFloat(@as(f32, @floatFromInt(vec.x)) * scalar));
    vec.y = @as(i32, @intFromFloat(@as(f32, @floatFromInt(vec.y)) * scalar));
}
...
pub fn emitParticles(particles: *[PARTICLE_NUMBER]Particle, target: *const Target) void {
    var emitted: usize = 0;
    const to_emit = PARTICLE_TO_EMIT + @as(i32, @intFromFloat((rand.random().float(f32) - 0.5) * PARTICLE_TO_EMIT_VARIABILITY));
    for (particles) |*particle| {
        if (particle.time_alive_sec < 0) {
            ...
            particle.speed += @as(i32, @intFromFloat((rand.random().float(f32) - 0.5) * PARTICLE_SPEED_VARIABILITY));
            particle.size += @as(i32, @intFromFloat((rand.random().float(f32) - 0.5) * PARTICLE_SIZE_VARIABLILIY));
...
pub fn updateBar(bar: *Bar) void {
    var nx = bar.pos.x + @as(i32, @intFromFloat(@as(f32, @floatFromInt(bar.vel)) * DELTA_TIME_SEC));
...
pub fn updateParticles(particles: *[PARTICLE_NUMBER]Particle) void {
    for (particles) |*particle| {
        if (particle.time_alive_sec >= 0) {
            ...
            particle.pos.x += @as(i32, @intFromFloat(@as(f32, @floatFromInt(particle.speed)) * math.cos(particle.angle)));
            particle.pos.y += @as(i32, @intFromFloat(@as(f32, @floatFromInt(particle.speed)) * math.sin(particle.angle)));
...
fn color_f32_to_u8(x: f32) u8 {
    return @as(u8, @intFromFloat(x * 255.0));
}
...
pub fn initialTargets() [TARGET_NUMBER]Target {
    ...
    for (&targets) |*target| {
        ...
        const t = @as(f32, @floatFromInt(idx_y)) / @as(f32, @floatFromInt(TARGET_Y_NUMBER));
